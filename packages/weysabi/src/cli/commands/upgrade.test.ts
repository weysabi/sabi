...
describe("upgradeCommand", () => {
  ...
  it("detects a generated project and upgrades files", async () => {
    const tmpDir = mkdtempSync("weysabi-upgrade-");
    try {
      ...
      expect(existsSync(resolve(projectDir, ".weysabi-template.json"))).toBeTrue();

      const marker = JSON.parse(
        readFileSync(resolve(projectDir, ".weysabi-template.json"), "utf-8")
      );
