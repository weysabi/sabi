...
function getPromptsDir(): string | null {
  ...
  const candidates = ["weysabi-prompts", "prompts", ".weysabi/prompts"];
...
export async function promptListCommand(): Promise<void> {
  ...
  if (!dir || !existsSync(dir)) {
    console.log("No prompts directory found. Set promptsDir in weysabi.json.");
    process.exit(1);
  }

  const files = readdirSync(dir).filter((f) => f.endsWith(".txt") || f.endsWith(".weysabi"));
  if (files.length === 0) {
...
export async function promptAddCommand(name: string, content?: string): Promise<void> {
  const dir = getPromptsDir();
  if (!dir) {
    console.error("Set promptsDir in weysabi.json first.");
    process.exit(1);
  }
