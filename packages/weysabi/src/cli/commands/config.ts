...
export async function configValidateCommand(): Promise<void> {
  p.intro("weysabi config validate");

  const loaded = loadConfig();
  if (!loaded) {
    p.log.warn("No weysabi.json config found. Run `weysabi init` to create one.");
  ...
  if (entries.length === 0) {
    p.log.error("No providers configured. Set env vars or run `weysabi init`.");
