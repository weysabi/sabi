...
export async function doctorCommand(): Promise<void> {
  console.log("weysabi doctor — System Diagnostics\n");
  ...
  checks.push([
    "Config file",
    config !== null,
    config ? config.path : "Not found — run `weysabi init`",
  ]);

  const pkgPath = new URL("../../../package.json", import.meta.url);
  try {
    const pkg = await import(pkgPath.toString());
    checks.push(["weysabi version", true, `v${pkg.version ?? "unknown"}`]);
  } catch {
    checks.push(["weysabi version", false, "Could not read package.json"]);
  }
