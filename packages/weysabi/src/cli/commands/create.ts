...
const weysabi = createWeysabi(
  ...
  {
    defaultModel: process.env.WEYSABI_DEFAULT_MODEL ?? "openai/gpt-4o-mini",
  }
);

const server = await createServer(weysabi, {
  apiKey: process.env.WEYSABI_API_KEY,
  adminApiKey: process.env.WEYSABI_ADMIN_API_KEY,
  controlPlane: true,
});
...
WEYSABI_API_KEY=dev-client-key

# Required for admin/control-plane routes such as /v1/projects.
WEYSABI_ADMIN_API_KEY=dev-admin-key

WEYSABI_DEFAULT_MODEL=openai/gpt-4o-mini
WEYSABI_CONTROL_DB=.weysabi/control.db
WEYSABI_PORT=3000
WEYSABI_HOST=0.0.0.0
`,
      },
      {
        path: ".gitignore",
...
- Control plane: \`/v1/projects\` with \`Authorization: Bearer $WEYSABI_ADMIN_API_KEY\`

Provider secrets and admin keys must stay server-side.
`,
      },
      {
        path: ".weysabi-template.json",
...
export async function POST(request: Request) {
  ...
  const response = await weysabi.complete({
    model: process.env.WEYSABI_DEFAULT_MODEL ?? "openai/gpt-4o-mini",
    messages,
  });
...
Bun.serve({
  ...
  async fetch(request) {
    ...
    if (request.method === "POST" && url.pathname === "/api/chat") {
      ...
      const response = await weysabi.complete({ model: process.env.WEYSABI_DEFAULT_MODEL ?? "openai/gpt-4o-mini", messages });
...
const adminHeaders = { Authorization: \`Bearer \${process.env.WEYSABI_ADMIN_API_KEY}\`, "content-type": "application/json" };
