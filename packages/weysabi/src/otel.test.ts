...
describe("createOtelPlugin", () => {
  it("creates a span on complete and ends it on response", async () => {
    ...
    weysabi.use(createOtelPlugin({ tracer: fakeTracer }));
    ...
    expect(attributes["weysabi.model"]).toBe("groq/llama-3.1-8b-instant");
    expect(attributes["weysabi.latency_ms"]).toBe(result.latencyMs);
    expect(attributes["weysabi.total_tokens"]).toBe(15);
  });
