...
describe("Sabi HTTP client", () => {
  ...
  it("streams managed conversation events", async () => {
    ...
    expect(requests[0]?.url).toBe(
      "https://weysabi.example/v1/projects/project-1/conversations/conversation-1/messages/stream"
    );
  });
  ...
  it("uses admin project routes and query parameters", async () => {
    ...
    expect(requests[0]?.url).toBe(
      "https://weysabi.example/v1/projects/project%201/runs?conversationId=conversation-1&status=success&limit=10"
    );
