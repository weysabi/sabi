...
export function createOtelPlugin(options: OtelPluginOptions): Plugin {
  const { tracer, serviceName = "weysabi" } = options;
  ...
  return {
    ...
    onCompleteRequest(request: CompleteRequest): CompleteRequest {
      const span = tracer.startSpan(`${serviceName}.complete`, {
        attributes: {
          "weysabi.model": request.model,
          "weysabi.has_fallbacks": (request.fallbacks?.length ?? 0) > 0,
          "weysabi.has_tools": (request.tools?.length ?? 0) > 0,
          "weysabi.has_schema": request.schema !== undefined,
        },
      });
    ...
    onCompleteResponse(response: CompleteResponse, request: CompleteRequest): CompleteResponse {
      ...
      if (span) {
        span.setAttribute("weysabi.latency_ms", response.latencyMs);
        span.setAttribute("weysabi.provider", response.provider);
        span.setAttribute("weysabi.estimated_cost_usd", response.estimatedCostUsd);
        if (response.usage) {
          span.setAttribute("weysabi.prompt_tokens", response.usage.promptTokens);
          span.setAttribute("weysabi.completion_tokens", response.usage.completionTokens);
          span.setAttribute("weysabi.total_tokens", response.usage.totalTokens);
        }
        span.end();
