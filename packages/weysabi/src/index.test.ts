...
describe("Weysabi", () => {
  describe("createWeysabi", () => {
    it("creates an instance without throwing", () => {
      ...
      expect(weysabi).toBeInstanceOf(Object);
      expect(weysabi.complete).toBeFunction();
    });
    ...
    it("accepts custom baseUrl", () => {
      ...
      expect(weysabi).toBeTruthy();
    });
  });

  describe("prompt management", () => {
    it("registers and retrieves a structured prompt", () => {
      const weysabi = createWeysabi({ groq: { apiKey: "key" } });
      ...
      const prompt = weysabi.prompts.get("classifier");
    ...
    it("returns undefined for unknown prompt", () => {
      const weysabi = createWeysabi({ groq: { apiKey: "key" } });
      expect(weysabi.prompts.get("nonexistent")).toBeUndefined();
    });
    ...
    it("lists registered prompts", () => {
      ...
      const list = weysabi.prompts.list();
    ...
    it("removes a prompt", () => {
      ...
      expect(weysabi.prompts.has("test")).toBeTrue();
      weysabi.prompts.remove("test");
      expect(weysabi.prompts.has("test")).toBeFalse();
    });
    ...
    it("renders a structured prompt with variables", () => {
      ...
      const messages = weysabi.prompts.render("greeter", { user: "Ben", word: "hello" });
    ...
    it("throws PromptNotFoundError for unknown prompt render", () => {
      const weysabi = createWeysabi({ groq: { apiKey: "key" } });
      expect(() => weysabi.prompts.render("missing", { a: "1" })).toThrow(PromptNotFoundError);
    });
    ...
    it("throws MissingPromptInputError for missing input", () => {
      ...
      expect(() => weysabi.prompts.render("test", { a: "1" })).toThrow(MissingPromptInputError);
    });
    ...
    it("runs a prompt through the pipeline", async () => {
      ...
      const result = await weysabi.prompts.run("classifier", { text: "I want a refund" });
    ...
    it("run overrides model from definition", async () => {
      ...
      const result = await weysabi.prompts.run(
        "test",
        { word: "hi" },
    ...
    it("run throws with no model set", async () => {
      ...
      await expect(weysabi.prompts.run("nope", {})).rejects.toThrow(/no default model/i);
    });

    it("run throws for unknown prompt", async () => {
      const weysabi = createWeysabi({ groq: { apiKey: "key" } });
      await expect(weysabi.prompts.run("nonexistent", {})).rejects.toThrow(PromptNotFoundError);
    });
    ...
    it("supports initial prompt definitions in options", () => {
      ...
      const prompt = weysabi.prompts.get("builtin");
      expect(prompt).toBeDefined();
    ...
    it("clear removes all prompts", () => {
      const weysabi = createWeysabi({ groq: { apiKey: "key" } });
      weysabi.prompts.register({ id: "a", messages: [{ role: "user", content: "A" }] });
      weysabi.prompts.clear();
      expect(weysabi.prompts.get("a")).toBeUndefined();
    });
  });
  ...
  describe("complete", () => {
    ...
    it("times out and throws", async () => {
      ...
      const weysabi = createWeysabi({ groq: { apiKey: "key" } }, { timeout: 100 });
