...
describe("createGuardrails integration", () => {
  ...
  it("blocks injection on complete", async () => {
    ...
    const promise = weysabi.complete({
      model: "groq/llama-3.1-8b-instant",
...
describe("weysabi.guardrail", () => {
  it("blocks input when custom guardrail fails", async () => {
    const weysabi = createWeysabi({ groq: { apiKey: "gsk_abc" } });
    weysabi.guardrail("no-secrets", {
      scope: "input",
