...
export function createWeysabiProvider(weysabi: Weysabi): ProviderV3 {
  return {
    specificationVersion: "v3",
    languageModel(modelId: string): LanguageModelV3 {
      return new WeysabiLanguageModel(weysabi, modelId);
    },
    embeddingModel(_modelId: string): never {
      throw new WeysabiError(
        "Embedding models are not supported in the AI SDK adapter. Use weysabi.complete() directly."
      );
    },
...
export class WeysabiLanguageModel implements LanguageModelV3 {
  ...
  private weysabi: Weysabi;

  constructor(weysabi: Weysabi, fullModelId: string) {
    ...
    this.weysabi = weysabi;
  }

  async doGenerate(options: LanguageModelV3CallOptions): Promise<LanguageModelV3GenerateResult> {
    if (options.tools && options.tools.length > 0) {
      throw new WeysabiError(
        "Tools are not supported in the AI SDK adapter. Use weysabi.complete() directly for tool calling."
      );
    }

    const result = await this.weysabi.complete({
      model: this.modelId,
  ...
  async doStream(options: LanguageModelV3CallOptions): Promise<LanguageModelV3StreamResult> {
    ...
    const streamIterable = await this.weysabi.stream({
      model: this.modelId,
      messages: convertV3Prompt(options.prompt),
      maxTokens: options.maxOutputTokens,
      temperature: options.temperature,
