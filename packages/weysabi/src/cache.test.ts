...
describe("weysabi cache integration", () => {
  it("caches complete responses", async () => {
    ...
    const result1 = await weysabi.complete(req);
    expect(result1.content).toBe("response-1");
    expect(callCount).toBe(1);

    const result2 = await weysabi.complete(req);
  ...
  it("cache miss calls provider", async () => {
    ...
    const r1 = await weysabi.complete({
      model: "groq/llama-3.1-8b-instant",
      messages: [{ role: "user" as const, content: "a" }],
    });
    const r2 = await weysabi.complete({
      model: "groq/llama-3.1-8b-instant",
  ...
  it("runs response plugins on cache hits without caching transformed output", async () => {
    ...
    const weysabi = createWeysabi({ groq: { apiKey: "key" } }, { cache: new InMemoryCache() });
    weysabi.use({
      name: "suffix",
