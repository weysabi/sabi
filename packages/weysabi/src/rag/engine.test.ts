...
function dbPath(): string {
  testId++;
  return `.weysabi/test-rag-engine-${testId}.db`;
}
...
describe("RagEngine", () => {
  ...
  it("clear with file path removes specific file", async () => {
    ...
    engine.clear(resolve(".weysabi/rag/files/remove.md"));
    expect(engine.stats().files).toBe(1);
    cleanup();
  });
  ...
  it("persists content across close and reopen", async () => {
    const path = `.weysabi/test-rag-persistence-${Date.now()}.db`;
  ...
  it("deduplicates across close and reopen", async () => {
    const path = `.weysabi/test-rag-dedup-${Date.now()}.db`;
    const engine1 = new RagEngine({
      dbPath: path,
      embeddingModel: "test/text-embedding",
      chunkSize: 100,
